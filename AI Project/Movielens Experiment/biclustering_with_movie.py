...
for row in test_data:
    ...
    print('Bi Cluster for this movie : ')
    movie = row[1]
    print(movie)
    movie_id = movie_ids.index(movie)
    print(movie_id)
    #print(labels)
    bicluster_index_movie = column_labels[movie_id]
    print(bicluster_index_movie)
    
    print('Other movie ids  in this cluster : ')
    print(bicluster_num_movies[bicluster_index_movie])
    print(len(bicluster_list_movies[bicluster_index_movie]))
    other_movie_ids_in_same_cluster=bicluster_list_movies[bicluster_index_movie]
    print(other_movie_ids_in_same_cluster)

    number_of_movies_rated_by_user=0
    sum_total_rating_1=0
    for i in other_movie_ids_in_same_cluster:
        if user_movie_matrix[user_id][i] > 0:
            print(i)
            print('movie has been rated')
            print(movie_id)
            print(user_movie_matrix[user_id][i])
            number_of_movies_rated_by_user+=1
            sum_total_rating_1+=user_movie_matrix[user_id][i]
    ...
    if(number_of_movies_rated_by_user > 0):
        rating_predicted = (rating_predicted + (sum_total_rating_1/number_of_movies_rated_by_user))/2
        print(rating_predicted)
